...
half4 LitPassFragmentSimple(Varyings input) : SV_Target
{
	...
	float4 noise = SampleAlbedoAlpha(uv, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap));

	float2 uv2 = input.uv;
	uv2.x = uv2.x * 0.0625 - _Time.y * 0.0052;
	uv2.y -= _Time.y * 0.23;

	float4 noise2 = SampleAlbedoAlpha(uv2, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap));
